...
function initLogin(){
  $('#playerLoginForm')?.addEventListener('submit',e=>{
    ...
    const d=getData();
    const p=d.players.find(x=>
      (x.email&&x.email.toLowerCase()===email) ||
      (x.contact&&x.contact.toLowerCase()===email)
    );

    if(!p){
      box.insertAdjacentHTML('afterend','<p class="login-error">No player found with that email — ask the club desk to add you first</p>');
      return;
    }

    if((p.licenseStatus||'pending')!=='active'){
      box.insertAdjacentHTML('afterend','<p class="login-error">Your account is not active yet — ask the club desk to activate your licence</p>');
      return;
    }

    if(!p.password){
      p.password=password;
      p.lastSeen=new Date().toLocaleString('en-GB');
      setData(d);
    }else if(p.password!==password){
      box.insertAdjacentHTML('afterend','<p class="login-error">Wrong password</p>');
      return;
    }

    localStorage.setItem(PLAYER,p.id);
